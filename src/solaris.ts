...
import { CelestialBody, createPlanet, createSun } from './celestialBody';
...
for (let i = 0; i < starCount; i++) {
	posArray[i] = (random() - 0.5) * starScale;
}
...
// Sun
const sun = createSun(listener, F0);
scene.add(sun.mesh);

// Celestial bodies
const celestialBodies: CelestialBody[] = [];
for (let i = 1; i < celestialCount; i++) {
	const body = (celestialBodies[i] = createPlanet(listener, F0 + F0 * i));
	initOrbit(body, i);
	scene.add(body.mesh);
}
